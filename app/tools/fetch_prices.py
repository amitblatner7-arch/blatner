...
DEPT_RULES = [
    # guess_dept() מחזיר את הקטגוריה של ההתאמה הראשונה ברשימה, לכן הסדר כאן
    # הוא מהעדיפות הגבוהה ביותר (נבדקת ראשונה) לנמוכה ביותר (נבדקת אחרונה,
    # לפני הנפילה ל"אחר"). מסונכרן ידנית עם victory_local.py — ראו תיקון 2026-07-23.
    # גלידה→גליד (בלי ה"), כדי לתפוס גם "גלידת..." בסמיכות (למשל "מאגדת גלידת שמנת").
    (r"קפוא|גליד|ארטיק|טילון", "קפואים"),
    # \b סביב "יין" — בלי זה "יין" תפס בטעות שמות מותג שמסתיימים ב-"ליין"/"פיין".
    (r"בירה|\bיין\b|וודקה|ויסקי|אלכוהול|ליקר|ערק", "אלכוהול"),
    (r"חיתול|מטרנה|סימילאק|מזון תינוקות", "תינוקות"),
    (r"שמפו|סבון|טיפוח|שיניים|דאודורנט|פארם|היגיינ|מרכך|תחבושת|טמפון|גילוח|אמבט|בושם|קוסמט", "פארם וטיפוח"),
    (r"ניקוי|ניקיון|כביסה|נייר|אשפה|אקונומיקה|ספוג|מגבון|שקית|כלים חד|צלחות חד|כוסות חד|מפיות|נר נשמה|נרות|גליל|מגבת|קטלן", "ניקיון"),
    # פריכ/קריספי/קראקר — בלעדיהם חטיפי תירס/בצל/עגבניה "פריכים" נתפסו בטעות כירקות.
    (r"חטיף|שוקולד|ממתק|וופל|עוגי|מסטיק|במבה|ביסלי|סוכרי|פופקורן|עוגיות|ביסקוויט|קוקוס|לוקום|פריכ|קריספי|קראקר", "חטיפים ומתוקים"),
    # \b סביב "חלב" — בלי זה "חלב" תפס בטעות "חלבון" (מוצרי חלבון/פרוטאין).
    (r"\bחלב\b|גבינ|יוגורט|ביצים|מעדן|שמנת|חמאה|קוטג|בולגרית|פטה|קממבר|מוצרלה", "מוצרי חלב"),
    # \b סביב "מים" — בלי זה "מים" תפס בטעות תוך כדי מילים כמו "בטעמים".
    (r"\bמים\b|משקה|מיץ|סודה|קולה|נקטר|תרכיז|נביעות|איזוטונ", "שתייה"),
    (r"לחם|מאפ|פית|לחמני|עוגה|חלה|בגט|קרואסון|בורקס|טוסט", "לחם ומאפים"),
    (r"שימור|רסק|רוטב|ממרח|חמוצים|זיתים|טונה|מטבוחה|אנשוב|סרדינ", "שימורים"),
    (r"בשר|עוף|הודו|דגים|נקניק|פסטרמה|קבב|שניצל|המבורגר|כבד|סלמון|בקר|קציצ|סלמי", "בשר ודגים"),
    # \b = גבול מילה ב-Python re (מקביל ל-\m/\M ב-Postgres), כדי ש"פרי"/"ירק"/"שום"
    # לא יתפסו כתת-מחרוזת בתוך "פריכ"/"פרימיום"/"ירקון"/"שומשום" וכד'.
    (r"\bפרי\b|\bירק\b|\bשום\b|פירות|עשבי|עגבני|מלפפון|בצל|תפוח אדמה|גזר|פלפל|חסה|כרוב|תפוח עץ|תפוז|לימון|אבוקדו|בננה|ענב|אבטיח|מלון|תות|קלמנטין|אשכולית|קישוא|חציל|תירס|פטריי", "ירקות ופירות"),
    (r"קמח|סוכר|אורז|פסטה|קטני|תבלין|שמן|דגני|קפה|תה |אגוז|טחינה|גרנולה|קורנפלקס|עדשים|קינואה|בורגול|שקד|בוטן|קשיו|פיסטוק|שיבולת שועל|משפר אפיה|שעועית|חומוס|אפונה|מלח", "יבשים"),
]
    ...
    for p in files:
        ...
        if re.search(r"price.*full|pricefull", bn, re.I):
            ...
            for item in t.iter("Item"):
                ...
                nm = clean_name(txt(item, "ItemName"))
                try:
